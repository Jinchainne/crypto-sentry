...
export default function DashboardPage() {
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-6">
      <div className="mb-8">
        <h1 className="text-2xl font-bold mb-1 text-[#F5F0E8]">Dashboard</h1>
        <p className="text-[#A8A09A] text-sm">Real-time crypto intelligence powered by Binance Skills Hub</p>
      </div>

      {/* Stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Active Signals", value: signals.length, icon: "📈", color: "#22c55e" },
          { label: "Tokens Tracked", value: market.length, icon: "🪙", color: "#F5A020" },
          { label: "Meme Tokens", value: memes.length, icon: "🔥", color: "#E8610A" },
          { label: "Skills Active", value: 6, icon: "🤖", color: "#E8610A" },
        ].map((stat) => (
          <div key={stat.label} className="stat-card">
            <div className="text-2xl mb-2">{stat.icon}</div>
            <div className="text-2xl font-bold" style={{ color: stat.color }}>{stat.value}</div>
            <div className="text-xs text-[#A8A09A] mt-1">{stat.label}</div>
          </div>
        ))}
      </div>
      ...
      <section className="mb-8">
        <h2 className="text-lg font-semibold mb-4 flex items-center gap-2 text-[#F5F0E8]">
        ...
        <div className="glass-card overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {market.map((item) => (
                <tr key={item.symbol} className="border-b border-white/[0.03] transition-colors">
                  <td className="px-4 py-3 text-[#A8A09A] font-mono">{item.rank}</td>
                  ...
                  <td className="px-4 py-3 text-right font-mono text-[#F5F0E8]/80">{item.price}</td>
                  <td className={`px-4 py-3 text-right font-mono font-semibold ${item.change24h >= 0 ? "text-green-400" : "text-red-400"}`}>
                    {formatPercent(item.change24h)}
                  </td>
                  <td className="px-4 py-3 text-right text-[#A8A09A] hidden md:table-cell">{item.volume24h}</td>
                  <td className="px-4 py-3 text-right text-[#A8A09A] hidden md:table-cell">{item.marketCap}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          {signals.map((sig) => (
            <div key={sig.id} className={`glass-card-hover p-5 ${sig.direction === "BUY" ? "signal-buy" : sig.direction === "SELL" ? "signal-sell" : "signal-hold"}`}>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center text-sm font-bold text-[#0E0804]" style={{ background: "linear-gradient(135deg, #E8610A, #F5A020)" }}>
                    {sig.token.slice(0, 2)}
                  </div>
                  <div>
                    <span className="font-bold text-lg text-[#F5F0E8]">{sig.token}</span>
                    <span className={`ml-2 text-xs px-2.5 py-1 rounded-full font-bold ${sig.direction === "BUY" ? "bg-green-500/20 text-green-400 border border-green-500/30" : sig.direction === "SELL" ? "bg-red-500/20 text-red-400 border border-red-500/30" : "bg-[rgba(245,160,32,0.2)] text-[#F5A020] border border-[rgba(245,160,32,0.3)]"}`}>
                      {sig.direction}
                    </span>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="bg-white/[0.03] rounded-lg p-3 border border-white/[0.05]">
                  <div className="text-[10px] text-[#A8A09A] mb-1 uppercase tracking-wider">Entry</div>
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-white/[0.06] rounded-full h-2">
                  <div className="h-2 rounded-full transition-all" style={{ width: `${sig.confidence}%`, background: "linear-gradient(90deg, #E8610A, #F5A020)" }} />
                </div>
          ...
          {memes.map((meme) => (
            <div key={meme.symbol} className="glass-card-hover p-4">
              ...
              <div className={`text-xs font-mono font-semibold ${meme.change24h >= 0 ? "text-green-400" : "text-red-400"}`}>
                {formatPercent(meme.change24h)} <span className="text-[#A8A09A] font-normal">24h</span>
              </div>
